...
const AdminProduct = () => {
  const { adminGetproducts, products, adminCodCharges } = useAdminBear((state) => state);
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState("");
  const [cod_charge, setCodCharge] = useState("");
          ...
          <table className="table w-full border-separate border-spacing-y-2 px-4">
            ...
            <thead>
              <tr className="text-slate-400">
                <th className="py-5 pl-6 font-bold uppercase text-[10px] tracking-widest border-b border-gray-50">
                  Product
                </th>
                <th className="font-bold uppercase text-[10px] tracking-widest border-b border-gray-50 text-center">
                  Price
                </th>
                <th className="font-bold uppercase text-[10px] tracking-widest border-b border-gray-50 text-center">
                  Discount
                ...
                <th className="pr-6 font-bold uppercase text-[10px] tracking-widest border-b border-gray-50 text-right">
                  Action
                </th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="py-4 pl-4 rounded-l-2xl border-none">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-black text-slate-800 text-base line-clamp-1">
                            {item?.product_name}
                          </div>
                    ...
                    <td className="text-center border-none">
                      <div className="flex flex-col gap-1 items-center justify-center">
                        {item?.discount > 0 ? (
                          <span className="badge badge-sm bg-emerald-50 text-emerald-600 border-none font-bold text-[10px]">
                            - {item?.discount}%
                          </span>
                        ) : (
                        ...
                        {item?.extra_discount > 0 && (
                          <span className="badge badge-sm bg-blue-50 text-blue-600 border-none font-bold text-[10px]">
                            + {item?.extra_discount}% Off
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="pr-4 text-right rounded-r-2xl border-none">
                      <motion.button
                        ...
                        onClick={() =>
                          navigate(
                            `/${import.meta.env.VITE_ADMIN_POST_URI}/products/details/${item._id}`,
                          )
                        }
