...
const ProductDetails = () => {
  ...
  const { userGetaProduct, userAddToCart } = useUserBear((state) => state);
  const { id } = useParams();
  ...
  const tabs = [
    ...
    { key: "how", label: "How to Use", content: selectedProduct.how_to_use },
    { key: "benefits", label: "Benefits", content: selectedProduct.benefits },
    {
      key: "recommended",
      label: "Recommended",
      content: selectedProduct.recommended,
    },
  ];
          ...
          <div>
            ...
            <div className="mt-4">
              <div className="flex items-center gap-3">
                <span className="text-2xl font-bold">
                  ₹{Math.round(selectedProduct.final_price)}
                </span>

                {(selectedProduct.discount ||
                  selectedProduct.extra_discount) && (
              ...
              {(selectedProduct.discount || selectedProduct.extra_discount) && (
                <p className="text-sm text-green-600 mt-1">
                  {selectedProduct.discount || 0}% +{" "}
                  {selectedProduct.extra_discount || 0}% OFF
                </p>
              )}

              {selectedProduct.cod_charges > 0 && (
                <p className="text-sm text-gray-500 mt-1">
                  COD Charges: ₹{selectedProduct.cod_charges}
                </p>
              )}
            </div>
            ...
            <div className="flex items-center gap-4 mt-8">
              ...
              <div className="flex items-center border rounded-xl overflow-hidden">
                ...
                <button className="px-4 py-2" onClick={() => setQty(qty + 1)}>
                  +
                </button>
              </div>
            </div>
            ...
            {loader ? (
              ...
              <div className="hidden sm:flex gap-4 mt-10">
                <button
                  ...
                  className="flex-1 bg-black text-white py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-gray-800 transition"
                >
