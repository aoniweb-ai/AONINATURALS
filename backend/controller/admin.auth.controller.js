...
export const adminUpdateCod_charges = async (req, res) => {
    try {
        const admin = req.admin;
        const { price } = req.body;
        if (!price.trim()) return res.status(200).json({ message: "Invalid credentials" });

        const updatedAdmin = await Admin.findOneAndUpdate({ _id: admin._id }, { cod_charges: parseInt(price.trim()) }, { new: true }).select('-password')
        if (!updatedAdmin) return res.status(400).json({ message: "Internal server error" });
        return res.status(200).json({ message: "Cod charges updated", admin: updatedAdmin });
    } catch (error) {
        console.log("error while updating cod charges ", error);
        return res.status(500).json({ success: false, message: "Internal server error" })
    }
}